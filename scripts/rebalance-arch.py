...
"""
Rebalance #arch page:
- Arch nodes: restore padding 12px→15px, gap 8px→10px (less cramped)
- Permission cards: add flex-direction:column + flex:1 spacer before bottom note
  so the ⚠️ token note and 回复语境 section stick to the card bottom
  eliminating the dead empty space below them
"""
...
<div class="section" id="arch">
  ...
  <!-- 三端架构：icon 内联，padding 15px，gap 10px -->
  <div class="arch-flow" style="margin-bottom:10px;flex-shrink:0;">

    <div style="flex:1;padding:15px 16px;display:flex;flex-direction:column;gap:10px;">
    ...
    <div style="flex:1;padding:15px 16px;border-left:1px solid #1e2530;display:flex;flex-direction:column;gap:10px;">
      <div style="display:flex;align-items:center;gap:9px;">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;flex:1;min-height:0;">
    ...
    <div class="perm-card" style="overflow:hidden;display:flex;flex-direction:column;">
      ...
      <!-- spacer：把 token-note 推到卡底 -->
      <div style="flex:1;min-height:12px;"></div>
      <div class="token-note" style="margin:0 16px 14px;padding:10px 14px;font-size:11px;">⚠️ 读文档 + 写批注必须用老板本人授权的 token，不能用 Bot 应用的 token——批注在飞书中显示为老板本人发出。</div>
    </div>

    <!-- 第二层: 代码层权限 — flex 列，回复语境贴底 -->
    <div class="perm-card" style="overflow:hidden;display:flex;flex-direction:column;">
...
def process(src):
    ...
    assert 'padding:15px 16px' in html, 'arch node padding missing'
    assert 'flex:1;min-height:12px' in html, 'spacer missing'
    assert 'display:flex;flex-direction:column' in html, 'perm-card flex missing'
